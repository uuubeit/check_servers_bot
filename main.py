...
bot=telebot.TeleBot('5981806403:AAGE8CGspACfJ_dOFq7Rk8tfIPkJaCVDumk')
fileMn=fileManager()
...
def start_message(message):
    if message.chat.id ==787420516:
        delMessages('second')
        sshMn.setEditer(showGoodHosts,showBadHosts,polligMessages)
        try:
            sshMn.start()
            setMessages('main',bot.send_message(787420516,text= '<code>'+'Server polling...'+'</code>',parse_mode='html'))
        except Exception:
            setMessages('main',bot.send_message(787420516,text= '<code>'+'Server polling...'+'</code>',parse_mode='html'))
            sshMn.checkAllHost()
...
def polligMessages():
    ...
    if  main != None and main.text!='Server polling...':
        setMessages('main',bot.edit_message_text(chat_id= 787420516,message_id= main.id,text= '<code>'+'Server polling...'+'</code>',parse_mode='html'))
    if getMessages('error')!=None:
        bot.delete_message(787420516,getMessages('error').id)
...
def showGoodHosts(availHosts=[]):
    ...
    try:
        setMessages('main',bot.edit_message_text(chat_id= 787420516,message_id= getMessages('main').id,text= '<code>'+line+'</code>',parse_mode='html', reply_markup=markup))
    except Exception:
        setMessages('main',bot.send_message(787420516,text= '<code>'+line+'</code>',parse_mode='html', reply_markup=markup))
...
def showBadHosts(unavailHosts=[]):
    ...
    if len(unavailHosts) != 0:
        ...
        if errorMessage == None:
            setMessages('error', bot.send_message(chat_id=787420516, text="<code>" + line + "</code>", parse_mode="html"))
        else:
            setMessages('error',bot.edit_message_text(chat_id=787420516, message_id=getMessages('error').id, text="<code>" + line + "</code>", parse_mode="html"))
    else:
        if errorMessage !=None:
            bot.delete_message(787420516,errorMessage.id)
            setMessages('error',None)
...
def addHost(message):
    if fileMn.addHost(message.text):
        setMessages('second',bot.send_message(787420516,'Complete'))
        time.sleep(1)
    else: 
        setMessages('second',bot.send_message(787420516,'Error'))
        time.sleep(1)
    delMessages('second')

def delMessages(name=str):  
    if name == 'main':
        bot.delete_message(787420516,getMessages('main').id)

    elif name == 'second':
        listMsg=getMessages('second')
        for message in listMsg:
            bot.delete_message(787420516,message.id)
